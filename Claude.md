...
- **Completed episodes**: two separate records mean "finished" and BOTH must be written. `historyStore` gets an entry (this is what the Profile logbook and the Completed screen read) and `podcastStore.markEpisodePlayed()` sets `episode.played`. Before this, only history was written, so `episode.played` was `false` on every episode ever — the flag existed and was preserved across refreshes but nothing ever set it. Both fire from the same two places in `usePlaybackController`: `handleEnd`, and the >=90% branch of `togglePlayPause`
  - The Completed screen (`src/screens/CompletedEpisodesScreen/`, reached from the PodcastDetail footer button) is sourced from **history**, not `episode.played`, so episodes finished before the flag existed still appear
  - `Episode.link` comes from RSS `<link>` and is what Share sends; `audioUrl` is a raw media file and is only the fallback. Podcasts subscribed before this need a pull-to-refresh to pick up `link`
- **Shared formatting**: every cross-screen formatter lives in `src/utils/`, never in a presenter. Presenters are private to their screen — importing one from another screen breaks the layering rule, and the repo is currently at **zero** such imports. Keep it that way:
...
- **Networking**: all outbound requests go through `fetchWithTimeout()` in `src/utils/fetchWithTimeout.ts` (15s default). It uses `AbortController` + `setTimeout`, **not `AbortSignal.timeout()`** — Node 18+ implements that static helper so Jest would pass, but React Native's fetch polyfill does not reliably provide it. Callers map aborts to a "timed out" message via `isTimeoutError()`. Never call bare `fetch()` in a service
- **Refreshing**: `RefreshService` owns the fetch → diff → store-update sequence. Screens must call `refreshAllPodcasts()` / `refreshPodcast()` rather than reimplementing it — `RefreshService.refreshAllPodcasts` is the only path that resets `lastRefreshTime`, so an inline copy leaves the 15-minute foreground throttle stale and the app re-fetches every feed seconds later
- **Lock screen / CarPlay**: `AudioPlayerService.loadEpisode(episode, podcast)` publishes now-playing metadata via `player.setActiveForLockScreen()`, and `unloadCurrentPlayer()` releases it. The podcast argument is optional so playback still works without metadata. This drives the iOS lock screen, Control Center, the Android media notification, **and CarPlay / Android Auto transport controls**, since both surface the same OS media session
...
- **Completed episodes**: there are two records of "finished", written together in `usePlaybackController` at both completion points (natural end, and pausing past 90%):
  - `historyStore` entries — the cross-podcast logbook on Profile, and the source for the per-podcast **Completed** screen
  - `episode.played` via `podcastStore.markEpisodePlayed()` — a per-episode flag for row indicators. It was dead for a long time: `updatePodcastEpisodes` carefully *preserved* it across refreshes while nothing ever set it, so every episode was `played: false` forever
  - `CompletedEpisodesScreen` reads **history**, not `played`, so it is populated by listening history that predates the flag being wired. Filter is `item.podcast.id === podcastId`
  - Its action menu keeps raw history entries index-aligned with the formatted list, because building a queue item or a share link needs the full `Episode`/`Podcast`, which the formatted display type doesn't carry
  - Sharing prefers `episode.link` (the RSS `<link>`, an episode web page) over `audioUrl`, which is a raw media file. Episodes cached before that field existed fall back to `audioUrl` until the podcast is refreshed
